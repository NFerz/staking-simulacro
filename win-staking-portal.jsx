...
<head>
  ...
  <title>WIN Staking Portal</title>
  <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 32 32'><circle cx='16' cy='16' r='16' fill='%2322c55e'/><text x='16' y='22' text-anchor='middle' fill='%23080b12' font-size='18' font-weight='bold' font-family='monospace'>W</text></svg>" />
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .tab-active { background: #22c55e; color: #080b12; }
          ...
          {yTicks.map((v, i) => {
            ...
            return (
              <g key={i}>
                ...
                <text x={pad.l - 6} y={y + 4} textAnchor="end" fill="#64748b" fontSize="10" fontFamily="'DM Mono', monospace">{pct(v)}</text>
              </g>
            );
          })}
          <path d={area} fill="url(#apy-grad)" />
          <path d={line} fill="none" stroke="#22c55e" strokeWidth="2" />
          {points.map((p, i) => (<circle key={i} cx={p.x} cy={p.y} r="3" fill="#0f172a" stroke="#22c55e" strokeWidth="1.5" />))}
          {data.map((d, i) => {
            const x = pad.l + (i / (data.length - 1)) * w;
            return (<text key={i} x={x} y={pad.t + h + 16} textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="'DM Mono', monospace">M{d.month}</text>);
          })}
        </svg>
      );
    }
    ...
    function Tooltip({ text, children }) {
      ...
      return (
        <span style={{ position: "relative", display: "inline-flex", alignItems: "center", cursor: "help" }} onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
          {children}
          {show && (
            <span style={{ position: "absolute", bottom: "calc(100% + 8px)", left: "50%", transform: "translateX(-50%)", background: "#1e293b", color: "#e2e8f0", padding: "8px 12px", borderRadius: "8px", fontSize: "12px", lineHeight: "1.5", whiteSpace: "nowrap", zIndex: 100, border: "1px solid #334155", boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}>
              {text}
            </span>
          )}
        </span>
    ...
    function App() {
      ...
      const handleStake = () => {
        ...
        setTimeout(() => {
          ...
          setTxDone(true);
          setAmount("");
          setTimeout(() => { setShowTx(false); setTxDone(false); }, 2000);
        }, 1500);
      };
      ...
      return (
        <div style={{ minHeight: "100vh", background: "#080b12", color: "#e2e8f0", fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, sans-serif" }}>

          {/* Nav */}
          <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 32px", borderBottom: "1px solid #111827", background: "#080b12ee", backdropFilter: "blur(12px)", position: "sticky", top: 0, zIndex: 50 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 28 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg, #22c55e, #15803d)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, color: "#080b12", fontFamily: "'DM Mono', monospace" }}>W</div>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                {["Sports Market", "Fantasy", "Investments", "Staking"].map((item) => (
                  <span key={item} style={{ padding: "7px 14px", fontSize: 13, fontWeight: item === "Staking" ? 600 : 400, color: item === "Staking" ? "#22c55e" : "#64748b", background: item === "Staking" ? "#22c55e12" : "transparent", borderRadius: 8, cursor: "pointer", transition: "color 0.2s", letterSpacing: "0.1px" }}>{item}</span>
                ))}
              </div>
            </div>
            {connected ? (
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ background: "#0f1724", border: "1px solid #1e293b", borderRadius: 10, padding: "8px 14px", fontSize: 12, fontFamily: "'DM Mono', monospace", color: "#94a3b8", display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#22c55e", animation: "pulse-glow 2s infinite" }} />
                  0x7a3F...e4Bc
                </div>
              </div>
            ) : (
              <button className="connect-btn" onClick={() => setConnected(true)}><WalletIcon />Connect Wallet</button>
            )}
          </nav>

          {/* Hero */}
          <div style={{ textAlign: "center", padding: "48px 32px 12px", position: "relative" }}>
            <div style={{ position: "absolute", top: 0, left: "50%", transform: "translateX(-50%)", width: 600, height: 300, background: "radial-gradient(ellipse, #22c55e08 0%, transparent 70%)", pointerEvents: "none" }} />
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 12 }}>
              <ShieldIcon />
              <span style={{ fontSize: 12, fontWeight: 600, color: "#22c55e", letterSpacing: "1.5px", textTransform: "uppercase", fontFamily: "'DM Mono', monospace" }}>Stake $WIN · Earn Rewards</span>
            </div>
            <h1 style={{ fontSize: 42, fontWeight: 700, letterSpacing: "-1.5px", lineHeight: 1.1, background: "linear-gradient(135deg, #f8fafc 0%, #94a3b8 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>WIN Staking</h1>
            <p style={{ fontSize: 15, color: "#64748b", marginTop: 12, maxWidth: 480, margin: "12px auto 0", lineHeight: 1.6 }}>Stake $WIN, receive sWIN, and earn rewards automatically. No complex locks, no manual compounding.</p>
          </div>

          {/* Protocol Stats */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, maxWidth: 900, margin: "28px auto 0", padding: "0 32px" }}>
            {[
              ...
              <div key={i} className="stat-card" style={{ animation: `slide-up 0.5s ${i * 0.08}s both` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
                  ...
                  <Tooltip text={s.sub}><InfoIcon /></Tooltip>
                </div>
                <div style={{ fontSize: 24, fontWeight: 700, fontFamily: "'DM Mono', monospace", color: s.accent ? "#22c55e" : "#f1f5f9", letterSpacing: "-0.5px" }}>{s.value}</div>
          ...
          {/* Main Content */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, maxWidth: 900, margin: "28px auto 0", padding: "0 32px", alignItems: "start" }}>
            {/* Action Panel */}
            <div className="action-card">
              <div style={{ display: "flex", gap: 4, padding: "16px 16px 0" }}>
                <div style={{ display: "flex", flex: 1, background: "#0f1724", borderRadius: 12, padding: 3 }}>
                  {["stake", "unstake"].map((t) => (
                    <button key={t} className={`tab-btn ${tab === t ? "tab-active" : "tab-inactive"}`} onClick={() => { setTab(t); setAmount(""); }}>{t === "stake" ? "Stake" : "Unstake"}</button>
                  ))}
                </div>
              </div>
              <div style={{ padding: "20px 20px 24px" }}>
                {!connected ? (
                  <div style={{ textAlign: "center", padding: "40px 20px" }}>
                    ...
                    <button className="connect-btn" onClick={() => setConnected(true)} style={{ margin: "0 auto" }}><WalletIcon />Connect Wallet</button>
                  </div>
                ) : (
                  <>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, fontSize: 12, color: "#64748b" }}>
                      ...
                      <span style={{ fontFamily: "'DM Mono', monospace", color: "#94a3b8" }}>{fmt(tab === "stake" ? walletBalance - userStaked : userStaked, 0)} WIN</span>
                    </div>
                    <div className="input-wrap">
                      <input type="number" className="amount-input" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} />
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <button className="max-btn" onClick={() => setAmount(String(maxAmount))}>MAX</button>
                        <div style={{ background: "#1e293b", borderRadius: 8, padding: "6px 12px", fontSize: 12, fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
                          <div style={{ width: 18, height: 18, borderRadius: "50%", background: "linear-gradient(135deg, #22c55e, #15803d)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 800, color: "#080b12" }}>W</div>
                          WIN
                        </div>
                      </div>
                    </div>
                    {amount && parseFloat(amount) > 0 && (
                      <div style={{ marginTop: 12, padding: "12px 14px", background: "#0d111b", borderRadius: 10, border: "1px solid #1e293b22", animation: "slide-up 0.3s" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#64748b", marginBottom: 6 }}>
                          <span>You will {tab === "stake" ? "receive" : "get back"}</span>
                          <span style={{ color: "#94a3b8", fontFamily: "'DM Mono', monospace" }}>{tab === "stake" ? fmt(parseFloat(amount) / sWINRate, 2) + " sWIN" : fmt(parseFloat(amount) * sWINRate, 2) + " WIN"}</span>
                        </div>
                        ...
                        {tab === "unstake" && (
                          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#64748b", marginTop: 6 }}>
                            <span style={{ display: "flex", alignItems: "center", gap: 4 }}><ClockIcon /> Cooldown</span>
                    ...
                    <button
                      className={`stake-btn ${!amount || parseFloat(amount) <= 0 || parseFloat(amount) > maxAmount ? "stake-btn-disabled" : tab === "stake" ? "stake-btn-active" : "unstake-btn-active"}`}
                      style={{ marginTop: 16 }}
                      disabled={!amount || parseFloat(amount) <= 0 || parseFloat(amount) > maxAmount || showTx}
                      onClick={tab === "stake" ? handleStake : handleUnstake}
                    >
                      {showTx ? (txDone ? "✓ Confirmed" : (<><div style={{ width: 16, height: 16, border: "2px solid currentColor", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />Confirming...</>)) : tab === "stake" ? (<><StakeIcon /> Stake WIN</>) : "Unstake WIN"}
                    </button>
                  </>
                )}
              </div>
                ...
                <div className="action-card" style={{ padding: 20 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    {[
                      ...
                      <div key={i} style={{ padding: "10px 0" }}>
                        ...
                        <div style={{ fontSize: 17, fontWeight: 700, fontFamily: "'DM Mono', monospace", color: s.accent ? "#22c55e" : "#f1f5f9", letterSpacing: "-0.3px" }}>{s.value}</div>
                  ...
                  {cooldowns.length > 0 && (
                    <div style={{ marginTop: 16, borderTop: "1px solid #1e293b", paddingTop: 14 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10, fontSize: 12, color: "#f59e0b" }}><ClockIcon /> Active Cooldowns</div>
                      {cooldowns.map((cd, i) => {
                        ...
                        return (
                          <div key={i} style={{ marginBottom: 8 }}>
                            ...
                            <div className="cooldown-bar"><div className="cooldown-fill" style={{ width: `${Math.min(elapsed * 100, 100)}%` }} /></div>
                          </div>
                        );
                      })}
                    </div>
              ...
              <div className="action-card" style={{ padding: 20 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 10, color: "#64748b" }}>
                    <span style={{ display: "flex", alignItems: "center", gap: 4 }}><span style={{ width: 8, height: 2, background: "#22c55e", borderRadius: 1, display: "inline-block" }} /> Total APY</span>
                  </div>
                </div>
                <APYChart data={monthlyAPY} width={400} height={140} />
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12, padding: "10px 14px", background: "#0d111b", borderRadius: 10, fontSize: 11, color: "#64748b" }}>
                  <span>M1: <span style={{ color: "#22c55e", fontFamily: "'DM Mono', monospace" }}>{pct(monthlyAPY[0].totalAPY)}</span></span>
                  <span>M6: <span style={{ color: "#22c55e", fontFamily: "'DM Mono', monospace" }}>{pct(monthlyAPY[5].totalAPY)}</span></span>
                  <span>M12: <span style={{ color: "#22c55e", fontFamily: "'DM Mono', monospace" }}>{pct(monthlyAPY[11].totalAPY)}</span></span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
              {[
                ...
                <div key={i} className="stat-card" style={{ animation: `slide-up 0.5s ${i * 0.1}s both` }}>
                  <div style={{ width: 28, height: 28, borderRadius: "50%", background: "#22c55e18", border: "1px solid #22c55e33", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: "#22c55e", fontFamily: "'DM Mono', monospace", marginBottom: 12 }}>{s.step}</div>
            ...
            <div className="action-card" style={{ padding: 20 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 1 }}>
                {[
                  ...
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "10px 14px", borderBottom: i < 6 ? "1px solid #1e293b22" : "none", fontSize: 12 }}>
              ...
              {faqs.map((f, i) => (
                <div key={i} className="faq-item">
                  <button className="faq-q" onClick={() => setFaqOpen(faqOpen === i ? null : i)}>
                    {f.q}
                    <span style={{ transform: faqOpen === i ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.3s" }}><ChevronDown /></span>
                  </button>
                  {faqOpen === i && (<div style={{ padding: "0 20px 16px", fontSize: 13, color: "#64748b", lineHeight: 1.7, animation: "slide-up 0.3s" }}>{f.a}</div>)}
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Footer */}
          <div style={{ borderTop: "1px solid #111827", padding: "20px 32px", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 11, color: "#334155", maxWidth: 900, margin: "0 auto" }}>
            <span>WIN Investments © 2025 — Powered by thirdweb</span>
            <div style={{ display: "flex", gap: 16 }}>
              {["Docs", "Contract", "Audit"].map((l) => (
                <span key={l} style={{ display: "flex", alignItems: "center", gap: 4, cursor: "pointer", color: "#475569" }}>{l} <ExternalLink /></span>
              ))}
            </div>
          </div>
          ...
          {/* Tx Overlay */}
          {showTx && (
            <div style={{ position: "fixed", inset: 0, background: "#080b12cc", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
              <div style={{ background: "#0f1724", border: "1px solid #1e293b", borderRadius: 20, padding: "40px 48px", textAlign: "center", animation: "slide-up 0.3s" }}>
                {txDone ? (
                  <>
                    <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#22c55e22", border: "2px solid #22c55e", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px", fontSize: 28 }}>✓</div>
                    <div style={{ fontSize: 16, fontWeight: 600, color: "#22c55e" }}>Transaction Confirmed</div>
                    <div style={{ fontSize: 12, color: "#64748b", marginTop: 6 }}>{tab === "stake" ? "Your WIN has been staked successfully" : "Unstake initiated — 7-day cooldown started"}</div>
                  </>
                ) : (
                  <>
                    <div style={{ width: 48, height: 48, border: "3px solid #22c55e33", borderTopColor: "#22c55e", borderRadius: "50%", animation: "spin 1s linear infinite", margin: "0 auto 16px" }} />
